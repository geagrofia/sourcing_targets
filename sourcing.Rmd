...
list(
  tar_target(cc_row, 2),
  tar_target(drought_threshold_l, 7),
  tar_target(drought_threshold_l_width, 3.5),
  tar_target(heat_threshold_l, 7),
  tar_target(heat_threshold_l_width, 3.5),
  tar_target(flood_threshold_l, 2),
  tar_target(flood_threshold_l_width, 1),
  tar_target(drought_threshold_u, 10),
  tar_target(drought_threshold_u_width, 5),
  tar_target(heat_threshold_u, 10),
  tar_target(heat_threshold_u_width, 5),
  tar_target(flood_threshold_u, 4),
  tar_target(flood_threshold_u_width, 2)
)
```
## Global spatial data
...
```{targets spatial_data_preparation_processing_1a}
list(
  tar_target(r_ISO_file, r_ISO_make_write_f(v_ISO, r_lc, v_ISO_extent, ISO)),
  tar_target(r_ISO, r_ISO_get_f(ISO, r_ISO_file))
)

```
```{targets spatial_data_preparation_processing_1b}
list(
  tar_target(r_ISO1_file, r_ISO1_make_write_f(v_ISO1, r_ISO, ISO)),
  tar_target(r_ISO1, r_ISO1_get_f(ISO, r_ISO1_file))
)

```

```{targets spatial_data_preparation_processing_1c}
list(
...
```{targets spatial_data_preparation_processing_2}
...
```{targets spatial_data_preparation_processing_3}

list(
  tar_target(r_crop_ISO_lc_file, r_crop_ISO_lc_file(r_lc_ISO, r_crop_ISO, ISO, crop)),
...
```{targets spatial_data_preparation_processing_4}
...
list(
  ...
  tar_target(
    ...
    v_crop_ISO_lc_rcl_agg_get_f(v_crop_ISO_lc_rcl_agg_file, ISO, crop, v_ISO1)
  )
)
...
```{targets spatial_data_preparation_plots}
list(
  tar_target(r_lc_ISO_plot, r_lc_ISO_plot_f(r_lc_ISO, world, v_ISO_extent, ISO)),
  tar_target(r_crop_ISO_plot, r_crop_ISO_plot_f(r_crop_ISO, v_ISO1, ISO, crop)),
  tar_target(r_crop_ISO_lc_plot, r_crop_ISO_lc_plot_f(r_crop_ISO_lc, v_ISO1, ISO, crop)),
  tar_target(r_crop_ISO_lc_rcl_plot, r_crop_ISO_lc_rcl_plot_f(r_crop_ISO_lc_rcl, v_ISO1, ISO, crop))
  )
...
list(
  tar_target(r_clim_mask_trim_file, r_clim_mask_trim_file_make_write_f(r_lc_ISO, r_clim_mask, ISO)),
...
list(
  tar_target(r_rainfallc_file, r_rainfallc_make_write_f(ISO, crop, cc_data, r_clim_mask_trim)),
  tar_target(r_rainfallf_file, r_rainfallf_make_write_f(ISO, crop, cc_data, r_clim_mask_trim)),
  tar_target(r_tempc_file, r_tempc_make_write_f(ISO, crop, cc_data, r_clim_mask_trim)),
  tar_target(r_tempf_file, r_tempf_make_write_f(ISO, crop, cc_data, r_clim_mask_trim))
)
```

```{targets climate_base_data_trim_2}
list(
  tar_target(r_onsetc_file, r_onsetc_make_write_f(ISO, crop, cc_data, r_clim_mask_trim)),
  tar_target(r_onsetf_file, r_onsetf_make_write_f(ISO, crop, cc_data, r_clim_mask_trim)),
  tar_target(r_durationc_file, r_durationc_make_write_f(ISO, crop, cc_data, r_clim_mask_trim)),
  tar_target(r_durationf_file, r_durationf_make_write_f(ISO, crop, cc_data, r_clim_mask_trim))
)
```
...
list(
  tar_target(r_droughtc_file, r_droughtc_make_write_f(ISO, crop, cc_data, r_clim_mask_trim)),
  tar_target(r_droughtf_file, r_droughtf_make_write_f(ISO, crop, cc_data, r_clim_mask_trim)),
  tar_target(r_heatc_file, r_heatc_make_write_f(ISO, crop, cc_data, r_clim_mask_trim)),
  tar_target(r_heatf_file, r_heatf_make_write_f(ISO, crop, cc_data, r_clim_mask_trim)),
  tar_target(r_floodc_file, r_floodc_make_write_f(ISO, crop, cc_data, r_clim_mask_trim)),
  tar_target(r_floodf_file, r_floodf_make_write_f(ISO, crop, cc_data, r_clim_mask_trim))
)
...
```{targets impact_data_preparation}
...
```{targets single_hazard_fuzzy_partition_matrices_l_predict}
...
```{targets single_hazard_fuzzy_partition_matrices_u_predict}
...
```{targets multiple_hazard_fuzzy_partition_matrices_l_predict}
...
```{targets multiple_hazard_fuzzy_partition_matrices_u_predict}
...
```{targets multiple_hazard_fuzzy_partition_matrices_mixed_predict_c}
...
```{targets multiple_hazard_fuzzy_partition_matrices_mixed_predict_f}
...
```{targets climate_risk_profile_data_preparation}
...
```{targets climate_risk_profile_data_plots}

list(
  tar_target(
    dB_profile_d_l_h_l_plot,
    dB_profile_d_l_h_l_plot_make_f(v_crop_ISO_lc_rcl_agg, dB_profile_summary, ISO, crop)
  ),
  tar_target(
    dB_profile_d_l_h_l_plot,
    dB_profile_d_l_h_l_plot_make_f(v_crop_ISO_lc_rcl_agg, dB_profile_summary, ISO, crop)
