---
title: "Mapping Sowing Date for Barley in Rwanda"
author: "A Farrow"
date: "`r Sys.Date()`"
output:
  html_document:  
...
# Create a dataset for Rwanda

In this section I create a dataset based on the historical season onset for Rwanda to apply the dynamic sowing date method.

The season onset has a width of 179 pixels and a height of 170 pixels, but this includes all of Africa so needs to be cropped to Rwanda

The cropped dataset has a sowing date set between 100 and 250 days, with the earliest sowing date in the north-east corner and the latest in the south-west.

I add dekadal precipitation for each observation for Rwanda.
...
# get precipitation for Rwanda
r_prec <-rast("D:/repos/sourcing_targets/data/input/chirps_all.tif")

# do a rough crop to boundaries of Rwanda
r_prec_cod <- crop(r_prec, ext(28, 31.5, -3.5, -0.5))
...
# get onset for Rwanda
r_hist_barley___onset <-rast("D:/repos/sourcing_targets/data/RWA/barley/hist_barley___onset.tif")
r_hist_barley___onset
plot(r_hist_barley___onset)

# do a rough crop to boundaries of Rwanda
r_cod_hist_barley___onset <- crop(r_hist_barley___onset, ext(28, 31.5, -3.5, -0.5))
...
# Phenology calculations for Rwanda
...
growth_stages <-
    read.csv(paste0("D:/repos/sourcing_targets/data/RWA/barley/sowing_date_barley_int.csv"))
...
# Mask for phenological periods for Rwanda
...
writeRaster(mth_gp_r, 
    "D:/repos/sourcing_targets/data/RWA/barley/mth_total_r.tif",
    overwrite = TRUE,
    filetype = "GTiff"
  )
...
writeRaster(dek_gp_r, 
    "D:/repos/sourcing_targets/data/RWA/barley/dek_total_r.tif",
    overwrite = TRUE,
    filetype = "GTiff"
  )
...
writeRaster(mth_es_r, 
    "D:/repos/sourcing_targets/data/RWA/barley/mth_est_r.tif",
    overwrite = TRUE,
    filetype = "GTiff"
  )
...
writeRaster(dek_es_r, 
    "D:/repos/sourcing_targets/data/RWA/barley/dek_est_r.tif",
    overwrite = TRUE,
    filetype = "GTiff"
  )
...
writeRaster(mth_vs_r, 
    "D:/repos/sourcing_targets/data/RWA/barley/mth_veg_r.tif",
    overwrite = TRUE,
    filetype = "GTiff"
  )
...
writeRaster(dek_vs_r, 
    "D:/repos/sourcing_targets/data/RWA/barley/dek_veg_r.tif",
    overwrite = TRUE,
    filetype = "GTiff"
  )
...
writeRaster(mth_fs_r, 
    "D:/repos/sourcing_targets/data/RWA/barley/mth_flw_r.tif",
    overwrite = TRUE,
    filetype = "GTiff"
  )
```
...
writeRaster(dek_fs_r, 
    "D:/repos/sourcing_targets/data/RWA/barley/dek_flw_r.tif",
    overwrite = TRUE,
    filetype = "GTiff"
  )
```
...
writeRaster(mth_ys_r, 
    "D:/repos/sourcing_targets/data/RWA/barley/mth_yld_r.tif",
    overwrite = TRUE,
    filetype = "GTiff"
  )
```
...
writeRaster(dek_ys_r, 
    "D:/repos/sourcing_targets/data/RWA/barley/dek_yld_r.tif",
    overwrite = TRUE,
    filetype = "GTiff"
  )
```
...
writeRaster(mth_rs_r, 
    "D:/repos/sourcing_targets/data/RWA/barley/mth_rip_r.tif",
    overwrite = TRUE,
    filetype = "GTiff"
  )
```
...
writeRaster(dek_gp_r, 
    "D:/repos/sourcing_targets/data/RWA/barley/dek_rip_r.tif",
    overwrite = TRUE,
    filetype = "GTiff"
  )
```
